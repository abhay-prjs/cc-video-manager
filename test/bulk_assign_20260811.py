"""One-off: bulk-assign the 21 Raw folders pending in #assignments as of 2026-08-11.
Replicates AssignEditorSelect.callback() exactly:
...
cleanup pass (bulk_assign_cleanup_20260811.py) run after confirming the queue drained.
"""
import json
import os
import time
...
ASSIGNMENTS = [
    # (message_id, editor) — first 10 oldest folders only (submitted >2h ago), 2026-08-11 batch
    ("1536363667132780578", "Jewel"),   # krea 2 / Claudia
    ("1536372688505806939", "Storm"),   # followpeek 14 videos / Mina
    ("1536398405348237372", "Naomi"),   # Phrasly batch 18 / Joshua
    ("1536451191897194518", "Josh"),    # Engagement 4 (Finance) / Ivan
    ("1536451741564932267", "Aki"),     # Engagement 3 / Ivan
    ("1536451772636340324", "Zyon"),    # PB7 / Ivan
    ("1536452194998681660", "Jewel"),   # Engagement 4 / Ivan
    ("1536471855857274900", "Storm"),   # phrasly batch 19 / Joshua
    ("1536472416916738058", "Naomi"),   # phrasly batch 20 / Joshua
    ("1536473937628758058", "Josh"),    # Composio 06 / Michael
]
